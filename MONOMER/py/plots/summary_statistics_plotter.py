...
if __name__ == '__main__':
    ...
    for ext in all_external_validations(main_labs=MAIN_LABS):
        ...
        print("Processing external validation " + str(ext.internal_label() + " - " + str(external_nick)))
        ...
        for measure in fold_measures:
            ...
            plot_path = SUMMARY_STAT_DIR + "/external/" + internal_label + "_" + external_nick + "/" + measure_nick
            measure_vals = [measure.compute_measures(solutions=s) for s in solutions]
            barplot_with_std_to_file(
                path=plot_path, measures=measure_vals, bar_names=hof_names, label_y=measure.name(), classes=classes)
