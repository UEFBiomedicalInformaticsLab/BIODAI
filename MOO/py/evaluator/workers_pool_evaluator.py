...
class WorkersPoolEvaluator:
    ...
    def evaluate_batch(
            self, individuals, objectives: Iterable[PersonalObjective],
            verbose=False, force_sequential=False) -> [WorkerResult]:
        if verbose:
            print("Evaluating batch with num workers: " + str(self.__n_workers))
        if self.__n_workers > 1 and not force_sequential:
            return self.evaluate_batch_parallel_with_init(individuals, objectives)
        else:
            return self.evaluate_batch_sequential(individuals, objectives)

    def evaluate_batch_sequential(self, individuals, objectives: Iterable[PersonalObjective]) -> [WorkerResult]:
        res = []
        for i in individuals:
            res.append(self.__worker_state.evaluate(self.pack_individual(i, objectives)))
        return res

    def evaluate_batch_parallel_with_init(self, individuals, objectives: Iterable[PersonalObjective]) -> [WorkerResult]:
        self.init_workers_pool_if_needed()
        packed_pop = self.pack_pop(individuals, objectives)
    ...
    @staticmethod
    def pack_individual(
            individual: PeculiarIndividualByListlike, objectives: Iterable[PersonalObjective]) -> WorkPackage:
        return WorkPackage(individual=individual, objectives=objectives)

    def pack_pop(self, pop, objectives: Iterable[PersonalObjective]) -> list[WorkPackage]:
        res = []
        for i in pop:
            res.append(self.pack_individual(i, objectives))
        return res
