...
from plots.plot_labels import ALL_NSGA_LABS, ALL_INNER_LABS, ALL_MAIN_LABS, has_classification_inner_model
            ...
            for adj in adjuster_regressors:
                if adj is None or m in ALL_NSGA_LABS:  # Adjustment of non-ga is not supported yet.
                    res.extend(DEFAULT_LOCATION_MANAGERS_ARCHIVE.all_seeds_hof_from_labels_cv(
                        ...
                        n_outer_folds=n_outer_folds, cv_repeats=cv_repeats, view_sets=view_sets, cox_fi=cox_fi,
                        generations=generations, population=population, hof_nick=hof_nick, adjuster_regressor=adj))
            ...
            for adj in adjuster_regressors:
                if adj is None or m in ALL_NSGA_LABS:  # Adjustment of non-ga is not supported yet.
                    res.extend(DEFAULT_LOCATION_MANAGERS_ARCHIVE.all_seeds_hof_from_labels_external(
                        ...
                        dir_from_label=dir_from_label, view_sets=view_sets, cox_fi=cox_fi,
                        generations=generations, population=population, hof_nick=hof_nick, adjuster_regressor=adj))
    for inn in inner_labs:
        for m in main_labs:
            if has_classification_inner_model(m) and dir_from_label.has_classification():
...
def flatten_hofs_for_dataset_cv(dataset_lab: str, main_labs=ALL_MAIN_LABS,
    ...
    return all_hof_combinations_cv(
        ...
        n_outer_folds=n_outer_folds, cv_repeats=cv_repeats, view_sets=view_sets, cox_fi=cox_fi, generations=generations,
        hof_nick=hof_nick)
