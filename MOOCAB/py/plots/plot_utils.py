...
def multi_line_plot_ax(
        ...
        x_max: float = None, y_max: float = None, font_size: int = FONT_SIZE, colors_by_label: bool = True,
    ...
    with plt.style.context({'font.size': font_size}):
        ...
        lines = []
        colors = line_colors(names=line_labels, colors_by_label=colors_by_label)
...
def format_tick(x, decimals: Optional[int]) -> str:
    if decimals is None or isinstance(x, int) or x.is_integer():
        return str(int(x))
    else:
