...
class PrefilteredMOOptimizerIncludingFI(MultiObjectiveOptimizer):
    ...
    def optimize(self, input_data: InputData, printer: Printer, n_proc=1, workers_printer=UnbufferedOutPrinter(),
        ...
        for v_name in input_data.view_names():
            printer.print("Computing active features for view " + str(v_name))
            v = input_data.view(view_name=v_name)
            active_features_by_view.append(
                self.__feature_selector.selection_mask(
                    x=v, outcomes=input_data.outcomes(), n_proc=n_proc, printer=printer))
        if very_verbose:
