...
#' Does not transpose. Creates directory if needed.
save_view <- function(view, name, directory, remove_zero_var_cols=FALSE) {
  if (!dir.exists(directory)){
    dir.create(directory, recursive = TRUE, showWarnings = FALSE)
  }
  if (remove_zero_var_cols) {
    view <- remove_zero_variance_cols(dat = view)
  }
...
load_survival_data <- function(tumours, event = "OS") {
  ...
  survival_data <- suppressWarnings(read_excel("work/kidney_ihc_det/raw/1-s2.0-S0092867418302290-mmc1.xlsx"))
...
remove_cols_by_prefix <- function(data, prefix) {
  columns_to_remove <- grep(prefix, names(data))
  data_reduced <- data[,-columns_to_remove]
  return (data_reduced)
}


remove_cols_by_names <- function(data, names) {
  return (data[ , !(names(data) %in% names)])
}


select_cols_by_prefix <- function(data, prefix) {
  columns_to_select <- grep(prefix, names(data))
  data_selected <- data[,columns_to_select]
  return (data_selected)
}

remove_col_prefix <- function(data, prefix) {
  names(data) <- sub(paste("^",prefix,sep=""), "", names(data))
  return (data)
}


#' Data should be convertible to a DataFrame. The function tries convert columns to numeric when possible.
#' The non-numeric columns are transformed in a sequence of columns for one-hot encoding. If an original column
#' contains NA, it is added as additional category to the one-hot encoding.
categories_to_one_hot <- function(data) {
  data <- DataFrame(data)
  char_columns <- sapply(data, is.character)
  for (col in names(char_columns)) {
    if (char_columns[col]) {
      col_data <- as.factor(data[[col]])
      if (anyNA(col_data)) {
        col_data <- addNA(col_data)
      }
      data[[col]] <- col_data
    }
  }
  # res <- one_hot(data)
  res <- one_hot(as.data.table(data))
  return (res)
}
