...
from util.dataframes import scale_df, n_col, non_finite_report, create_df_with_repeated_value, n_row
...
def cox_merge_x_y(x: DataFrame, y: DataFrame):
    ...
    res = pd.concat([x.reset_index(drop=True, inplace=False), events, times], axis=1)
    return res
...
class LifelinesModel(SurvivalModel):
        ...
        try:
            if ignore_warns:
                ...
                estimator = fitter.fit(df=df, duration_col=SURVIVAL_DURATION_STR,
                                       event_col=SURVIVAL_EVENT_STR, show_progress=False,
                                       fit_options={"step_size": self.__step_size})
            return LifelinesPredictor(estimator=estimator, scaler=scaler)
