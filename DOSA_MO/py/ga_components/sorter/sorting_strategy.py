...
class SortingStrategyNsga3CI(SortingStrategyWithSorter):
    """Uses NSGA3 sorting with clone index and random tournament for generating offsprings."""

    def __init__(self, num_objectives: int, max_reference_points: int, selection: Selection = DEFAULT_SELECTION):
        SortingStrategyWithSorter.__init__(
            self=self,
            sorter=SorterWithCloneIndex(
                inner_sorter=Nsga3(num_objectives=num_objectives, max_reference_points=max_reference_points)),
            tournament=RandomTournament(),
            sort_after_selection=True,
            selection=selection)
