...
class GAStrategy(ABC):
    ...
    def tournament(self, pop: [PeculiarIndividual], k: int) -> Sequence[PeculiarIndividual]:
        return self.__sorting_strategy.tournament(pop=pop, k=k)

    def clone_repurposing(self, pop: [PeculiarIndividual]) -> Sequence[PeculiarIndividual]:
    ...
    def select(self, pop: [PeculiarIndividual], pop_size: int) -> Sequence[PeculiarIndividual]:
        return self.__sorting_strategy.select(pop=pop, pop_size=pop_size)


def one_with_prob(prob):
    """Uses module random for random choice."""
