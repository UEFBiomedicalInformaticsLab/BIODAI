...
def anova_filter_one_feature(x, y, p_val=DEFAULT_P_VAL, verbose: bool = True, ignore_warns: bool = True) -> bool:
    try:
        if ignore_warns:
...
def anova_filter_one_feature_checked(
        x, y, p_val=DEFAULT_P_VAL, verbose: bool = True, ignore_warns: bool = True) -> bool:
...
def filter_anova_mask(
        view, outcome, p_val: float = DEFAULT_P_VAL, n_proc: int = 1, verbose: bool = False) -> List[bool]:
...
def filter_anova(view, outcome, p_val: float = DEFAULT_P_VAL, n_proc: int = 1):
    res = view.copy()
