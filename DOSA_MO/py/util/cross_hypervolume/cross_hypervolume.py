...
def cross_dimension_contribution(
    ...
    n_proc = min(n_proc,
                 max_procs(n_intervals=cartesian_product_size(intervals_lists), n_solutions=len(train_hyperboxes)))
    if n_proc <= 1:
...
def hypervolume(hyperboxes: Sequence[Hyperbox0B],
    ...
    n_proc = min(n_proc, max_procs(
        n_intervals=cartesian_product_size(intervals_lists), n_solutions=n_hyperboxes))
    if verbose:
