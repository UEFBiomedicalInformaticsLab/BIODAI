...
from model.classification.tree import DEFAULT_TREE_MIN_SAMPLES_LEAF
from model.importance_extractor import SklearnImportanceExtractor, OffImportanceExtractor, OnImportanceExtractor
from model.coef_extractor import SklearnCoefExtractor, OffCoefExtractor, OnCoefExtractor
from model.pipe_wrapper import PipeWrapper
from model.regression.regression_model_with_coef import SklearnRegressorModelWrapper, \
    SKLearnRegressionModelFactoryWithExtractor
...
class CompositeRegressorFactory(SKLearnRegressionModelFactoryWithExtractor):
    """Imputation is always applied by the created PipeWrapper."""
...
class SVRegressorModel(CompositeRegressorModelWrapper):
...
class JustTreeRegressorCreator(JustSKLearnRegressorCreator):
    ...
    def __init__(self, criterion: str, ccp_alpha: float = 0.0, min_samples_leaf: int = DEFAULT_TREE_MIN_SAMPLES_LEAF):
