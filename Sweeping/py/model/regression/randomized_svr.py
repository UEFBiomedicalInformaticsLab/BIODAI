...
class RandomizedSVRModel(RegressorSVModel):
    __verbosity: int
    ...
    def fit(self, x, y: Sequence[float], sample_weight: Optional = None) -> SVRegressor:
        ...
        for c, epsilon in zip(cs, epsilons):
            model = SVRegressorModel(c=c, epsilon=epsilon)
        ...
        return SVRegressorModel(c=best_c, epsilon=best_epsilon).fit(x, y, sample_weight=sample_weight)
