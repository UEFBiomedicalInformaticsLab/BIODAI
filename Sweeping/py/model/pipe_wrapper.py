...
class PipeWrapper:
    """Applies imputer then model. It supports also scaling and weights if they are set as active."""
    ...
    def fit(self, x, y, sample_weight: Optional = None):
        ...
        if self.__supports_weights:
            if self.__scale:
                res = pipe.fit(X=x, y=y, **{
                    SCALER_NICK+"__sample_weight": sample_weight,
                    self.__model_name+"__sample_weight": sample_weight})
            else:
                res = pipe.fit(X=x, y=y, **{self.__model_name + "__sample_weight": sample_weight})
        else:
            res = pipe.fit(X=x, y=y)
        try:
            check_is_fitted(res)
        except Exception as e:
            print("Predictor does NOT look fitted to sklearn:", e)
            print("Type:", type(res))
            print("Steps:", res)
        return res
