...
from model.survival.survival_model import SurvivalSVModel
...
class CoxLasso(SurvivalSVModel):
    __l1_ratio: float
    __standardize: bool
    __max_iter: int
    __verbose: bool
    ...
    def fit(self, x, y: DataFrame, sample_weight: Optional = None) -> SVPredictor:
        ...
        if n_col(x) > 0:
            ...
            train_imputer.fit(x)
            x_train = train_imputer.transform(x_train)
        ...
        with warnings.catch_warnings():
            ...
            sksurv_y_train = survival_df_to_sksurv(survival_df=y_train)
            try:
                fitter.fit(X=x_train, y=sksurv_y_train)
        ...
        for alpha in estimated_alphas:
            ...
            if temp_c > best_c:
                best_alpha = alpha
                best_c = temp_c
        best_model = SksurvModel(penalizer=best_alpha, l1_ratio=self.__l1_ratio, standardize=self.__standardize,
                                 max_iter=self.__max_iter, verbose=False)
