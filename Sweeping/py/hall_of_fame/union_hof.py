...
class UnionHof(HallOfFame):
    ...
    def name(self) -> str:
        return "union of " + iterable_to_string(li=[h.name() for h in self.__inner], brackets=False)

    def nick(self) -> str:
        return "union" + iterable_to_string(li=[h.nick() for h in self.__inner], compact=True, brackets=True)
