...
from model.sv_model import InputTransformer
...
class FitnessAdjusterInputTransformer(InputTransformer):
    ...
    def __str__(self) -> str:
        ...
        res = []
        if len(keeping) > 0:
            res.append("keeping " + iterable_to_string(li=keeping, brackets=False))
        if len(dropping) > 0:
            res.append("dropping " + iterable_to_string(li=dropping, brackets=False))
        return iterable_to_string(li=res, brackets=False, separator=" and")
...
class FitnessAdjusterLearner(NickNamed):
    ...
    def __init__(self, model: RegressorSVModel, use_bootstrap_mean: bool = False):
        self.__model = model
