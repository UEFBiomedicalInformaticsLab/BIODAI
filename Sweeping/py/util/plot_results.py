...
from plots.hofs_plotter.palette import DEFAULT_PALETTE
from plots.plot_utils import smart_save_fig, DEFAULT_DECIMALS, format_tick_labels, font_size_by_lines
...
def alpha_by_n_points(n_points: int, interpolate: bool = False) -> float:
    if n_points == 0:
        return 1.0
    else:
        numerator = 2000.0
        if interpolate:
            numerator = numerator / 8.0
        return min(numerator / n_points, 1.0)


def mid(element: Union[Interval, float]) -> float:
...
def plot_class_ellipse_to_ax(ax: Axes, x: Sequence[float | Interval], y: Sequence[float | Interval], color):
    xf = [mid(xi) for xi in x]
    yf = [mid(yi) for yi in y]
    xf, yf = remove_nan(x=xf, y=yf)
    centroid = (KahanSummer.mean(xf), KahanSummer.mean(yf))
    xy = numpy.array([(xf_i, yf_i) for xf_i, yf_i in zip(xf, yf)])
    # Calculate the covariance matrix and its eigenvalues and eigenvectors
    cov_matrix = np.cov(xy.T)
    eigenvalues, eigenvectors = np.linalg.eig(
        cov_matrix)  # Calculate the angle of rotation and lengths of the axes
    angle = np.arctan2(eigenvectors[1, 0], eigenvectors[0, 0])
    width, height = 2 * np.sqrt(eigenvalues)  # 2*sqrt to get the actual lengths# Plot the points and the ellipse
    ellipse = Ellipse(xy=centroid, width=width * 2, height=height * 2, angle=np.degrees(angle),
                      edgecolor=color, fc="None", lw=2)
    ax.add_patch(ellipse)


def multiclass_scatter_to_ax(ax: Axes,
    ...
    font_size = font_size_by_lines(n_lines=n_classes, initial_size=font_size)
    with plt.style.context({'font.size': font_size}):
        if class_labels is None:
            class_labels = []
            for i in range(n_classes):
                class_labels.append(str(i))
        if colors is None or len(colors) < n_classes:
            colors = DEFAULT_PALETTE.colors(n_colors=n_classes)
        if alpha is None:
            tot_points = sum([len(x_i) for x_i in x])
            alpha = alpha_by_n_points(tot_points, interpolate=interpolate)
        ...
        for i in range(n_classes):
            ...
            if len(x_i) > 0 and has_intervals(x=x_i, y=y_i):
                plot_ellipses_to_ax(
                    ax=ax, x_intervals=x_i, y_intervals=y_i, color=class_color, alpha=dots_alpha,
                    label=class_labels[i], min_width=min_width_for_resize, min_height=min_height_for_resize)
            else:
                if False and interpolate and tot_points > 10000:
                    # Colors would blend, so we go for black and white. Not using this at the moment.
                    class_color = 'black'
                    dots_alpha = dots_alpha / 4
                ax.scatter(x=x_i, y=y_i, c=[class_color], label=class_labels[i], alpha=dots_alpha, edgecolors='none')
            if interpolate:
                plot_interpolation_to_ax(ax=ax, x=x_i, y=y_i, color=colors[i])
            if add_ellipses:
                plot_class_ellipse_to_ax(ax=ax, x=x_i, y=y_i, color=colors[i])
        border_size = 0.03

        if x_min is not None:
            x_min_to_set = x_min
        else:
            x_min_computed = x_min_for_resize - (border_size * (x_max_for_resize - x_min_for_resize))
            x_min_to_set = min(x_min_computed, ax.get_xlim()[0])
            # To avoid reducing limits when there are other subplots.
        if math.isfinite(x_min_to_set):
            ax.set_xlim(left=x_min_to_set)

        if x_max is not None:
            x_max_to_set = x_max
        else:
            x_max_computed = x_max_for_resize + (border_size * (x_max_for_resize - x_min_for_resize))
            x_max_to_set = max(x_max_computed, ax.get_xlim()[1])
        if math.isfinite(x_max_to_set):
            ax.set_xlim(right=x_max_to_set)

        if y_min is not None:
            y_min_to_set = y_min
        else:
            y_min_computed = y_min_for_resize - (border_size * (y_max_for_resize - y_min_for_resize))
            y_min_to_set =min(y_min_computed, ax.get_ylim()[0])
        if math.isfinite(y_min_to_set):
            ax.set_ylim(bottom=y_min_to_set)

        if y_max is not None:
            y_max_to_set = y_max
        else:
            y_max_computed = y_max_for_resize + (border_size * (y_max_for_resize - y_min_for_resize))
            y_max_to_set =max(y_max_computed, ax.get_ylim()[1])
        if math.isfinite(y_max_to_set):
            ax.set_ylim(top=y_max_to_set)
        ...
        if n_classes > 1:
            ...
            for i, lh in enumerate(leg.legend_handles):
                ...
                # lh.set_color(colors[i])  # Does not work since the handles are not in the order in which they are created.
        if x_label is not None:
...
def multiclass_intervals_scatter_to_ax(
        ...
        add_ellipses: bool = ADD_ELLIPSES_DEFAULT):
    """x and y are one Sequence for each class."""
    multiclass_scatter_to_ax(ax=ax, x=x, y=y, x_label=x_label, y_label=y_label, class_labels=class_labels,
