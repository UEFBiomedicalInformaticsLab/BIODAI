...
def values_on_d(hyperboxes: Sequence[Hyperbox0B], d: int) -> list[float]:
    return [h.intervals()[d].b() for h in hyperboxes]


def d_max_values(hyperboxes: Sequence[Hyperbox0B], d: int) -> Sequence[float]:
...
def d_midpositions(hyperboxes: Sequence[Hyperbox0B], d: int) -> Sequence[float]:
...
def d_intervals(hyperboxes: Sequence[Hyperbox0B], d: int) -> Sequence[Interval]:
    d_max_vals = d_max_values(hyperboxes=hyperboxes, d=d)
    res = []
    lower = 0.0
    for upper in d_max_vals:
...
def cross_hypervolume_on_front(
        train_hyperboxes: Sequence[Hyperbox0B], test_hyperboxes: Sequence[Hyperbox0B], verbose: bool = True,
        n_proc: Optional[int] = DEFAULT_HV_PROCS) -> float:
    ...
    dimension_contributions = [
        cross_dimension_contribution(train_hyperboxes, volume_ratios, d, n_proc=n_proc) for d in range(n_dimensions)]
    if verbose:
...
def cross_non_dominated_hyperboxes(
        train_hyperboxes: Sequence[Hyperbox0B], test_hyperboxes: Sequence[Hyperbox0B]
) -> tuple[Sequence[Hyperbox0B], Sequence[Hyperbox0B]]:
...
def cross_hypervolume(train_hyperboxes: Sequence[Hyperbox0B], test_hyperboxes: Sequence[Hyperbox0B],
                      n_proc: Optional[int] = DEFAULT_HV_PROCS) -> float:
    """The two list of hyperboxes must have the same length and the i-th element of both list must refer to the
    same solution, with performance on the train and on the test datasets.
    ...
    return cross_hypervolume_on_front(train_hyperboxes=front_train_hb, test_hyperboxes=front_test_hb, n_proc=n_proc)
