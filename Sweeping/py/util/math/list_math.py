...
def dot_product(list_a: Sequence, list_b: Sequence, summer_class=KahanSummer):
    return summer_class.sum([x*y for x, y in zip(list_a, list_b)])


def list_add(list_a: Union[list, ListLike], list_b: Union[list, ListLike]):
...
def list_and(list_a: Sequence[bool], list_b: Sequence[bool]):
...
def indices_of_true(elems: Sequence) -> list:
...
def top_k_positions(elems: Union[list, ListLike], k: int) -> ndarray:
