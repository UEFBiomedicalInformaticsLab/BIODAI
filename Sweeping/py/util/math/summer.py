...
class Summer:
    ...
    def add_all(self, elems: Iterable[float], skip_nan: bool = SKIP_NAN_DEFAULT):
        if skip_nan:
            for e in elems:
                if not math.isnan(e):
                    self.add(e)
    ...
    @classmethod
    def sum(cls, elems: Iterable[float], skip_nan: bool = SKIP_NAN_DEFAULT):
        summer = cls.create()
        summer.add_all(elems, skip_nan=skip_nan)
        return summer.get_sum()

    @classmethod
    def mean(cls, elems: Iterable[float], skip_nan: bool = SKIP_NAN_DEFAULT):
        summer = cls.create()
        num = 0
        for e in elems:
            to_add = True
            if skip_nan and math.isnan(e):
                to_add = False
            if to_add:
                summer.add(e)
                num += 1
        return summer.get_sum()/num
...
class KahanSummer(Summer):
    ...
    def add(self, x: float) -> None:
        if isinstance(x, numbers.Number):
            self.add_unchecked(x=x)
        else:
            raise ValueError("Passed value is not an accepted number: " + str(x))

    def add_unchecked(self, x: float) -> None:
        """This version does not check if x is a number, which is quite slow."""
        y = x - self.__c
    ...
    @classmethod
    def mean_unchecked(cls, elems: Iterable[float], skip_nan: bool = SKIP_NAN_DEFAULT) -> float:
        """This version does not check if x is a number, which is quite slow."""
        summer = KahanSummer()
        num = 0
        for e in elems:
