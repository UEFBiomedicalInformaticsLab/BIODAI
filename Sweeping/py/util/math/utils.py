...
def ceil_division(num: int, den: int) -> int:
    ...
    Divides num by den returning the ceil in case the result is fractional. More accurate than Math.ceil because it
    does not use floats."""
    return -(num // -den)


def to_float(x) -> float:
    """Converts to float, if input is malformed returns NaN."""
    try:
        return float(x)
    except ValueError:
        return np.nan
...
def to_float_alternative(x) -> float:
    """Converts to float, if input is malformed returns NaN.
    It avoids the slow mechanism of exception handling by checking in advance if x can be converted.
    Still, does not seem particularly faster in practice."""
...
def to_np_float64(x):
    """Converts to np.float64, if input is malformed returns NaN.
    np.float64(x) has been tested slower than float(x)."""
    try:
        return np.float64(x)
    except ValueError:
        return np.nan
