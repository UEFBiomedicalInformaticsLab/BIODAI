...
class SparseBoolList(BoolListLike, ABC):
    ...
    def __dot_product_with_sparse(self, sparse: ListLike):
        res = 0
...
class SparseBoolListBySet(SparseBoolList):
    ...
    def __getitem__(self, key: Union[int, slice]) -> Union[bool, Sequence[bool]]:
        """Supports integer key or slice key."""
        if isinstance(key, (int, np.integer)):
            return self.__get_one_item(key=key)
        if isinstance(key, slice):
            indices = range(*key.indices(self.__size))
            return [self.__get_one_item(i) for i in indices]
            # TODO Could return a sparse list for higher efficiency,
            #  but getting the slice size in advance can be tricky.
        raise TypeError()
...
def dot_product(list_a: Union[ListLike, ndarray], list_b: Union[ListLike, ndarray], summer_class=KahanSummer):
...
def jaccard_score(y_true: BoolListLike, y_pred: BoolListLike, zero_division=1.0) -> float:
    intersection = 0
...
def union_of_pair_sparse(list_a: SparseBoolList, list_b: SparseBoolList) -> SparseBoolListBySet:
    ...
    if smart_sum(list_a) < smart_sum(list_b):
        small = list_a
        big = list_b
    else:
        small = list_b
...
def union_of_pair_sparse_a(list_a: SparseBoolList, list_b: ListLike) -> Union[ListLike, list]:
    """If lengths are different, the result has the bigger length."""
    if isinstance(list_b, SparseBoolList):
...
def union_of_pair(list_a: ListLike, list_b: ListLike) -> Union[ListLike, list]:
    """If lengths are different, the result has the bigger length."""
    if isinstance(list_a, SparseBoolList):
