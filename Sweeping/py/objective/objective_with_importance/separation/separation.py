...
class Separation(StructuralObjectiveComputerWithImportance, ABC):
    ...
    def compute_from_structure(self, hyperparams, hp_manager: Optional[MvHyperparamManager],
                               data: ModelReadyInputData) -> CVResult:
        """This could benefit from memoization or similar of the data adjustment."""
        if data.n_features() != hp_manager.n_used_features(hyperparams):
            raise ValueError("data features: " + str(data.n_features()) +
                             "\nUsed features from individual: " + str(hp_manager.n_used_features(hyperparams)))
        data = adjust_input_data(input_data=data).model_ready()  # This could benefit from memoization or similar.
        y = data.outcome_data()
        labels = unique_labels(y)
        n_labels = len(labels)
        x = data.collapsed_views().to_dataframe()
