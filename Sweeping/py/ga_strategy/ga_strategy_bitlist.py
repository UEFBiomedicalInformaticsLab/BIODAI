...
def create_individual_features(ind_size: int, num_features: NumFeatures) -> SparseBoolListBySet:
    """Does not use feature importance."""
    res_list = SparseBoolListBySet(min_size=ind_size)
    n_true_bits = num_features.extract(individual_size=ind_size)
    iterations = min(n_true_bits, ind_size)
    extracted = random.sample(range(ind_size), k=iterations)
    res_list.set_all_true(extracted)
    return res_list
...
class GAStrategyBitlist(GAStrategy):
    ...
    def __init__(self, input_data: InputData, mating_prob: float, mutation_frequency: float,
        ...
        """If active_features is None, all features are considered active."""
        super().__init__(
            evaluator=MaskEvaluator(
                ...
                compute_feature_importance=mutation.uses_personalized_feature_importance(),
                seed=random_seed(),
        ...
        if feature_importance is None:
            n_feat = input_data.n_features_per_view()
            feature_importance = {
                v: UniformDistribution(size=n_feat[v]).as_cached()
                for v in input_data.predictive_view_names()}
        self.__collapsed_feature_importance = collapse_feature_importance_dict(
    ...
    def create_individual(self) -> PeculiarIndividualSparse:
        ...
        iterations = min(n_true_bits, self.__non_zero_fi_num)
        for i in range(0, iterations):
            proceed = True
            while proceed:
                extracted = self.__collapsed_feature_importance.extract()
                if  not res_list[extracted]:
    ...
    def mutate(self, individual: Individual) -> tuple[Individual]:
        return self.__mutation.mutate(
            individual=individual, frequency=self.mutation_frequency())
