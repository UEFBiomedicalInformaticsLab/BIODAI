...
from util.dataframe.dataframes import has_non_finite, has_non_finite_error
from util.math.list_math import list_not, num_of_true, list_or
...
def filter_low_variance_mask(view: DataFrame, low_var: float = LOW_VARIANCE) -> Sequence[bool]:
    return [v > low_var for v in view.var()]


def filter_low_variance(view: DataFrame, low_var: float = LOW_VARIANCE) -> DataFrame:
    support = filter_low_variance_mask(view, low_var=low_var)
    res = view.iloc[0:, support]
    return res


def filter_zero_variance_mask(view: DataFrame) -> Sequence[bool]:
...
def filter_view(view, outcome: DataFrame, printer: Printer, p_val=DEFAULT_P_VAL, n_proc=1):
    outcome_array = outcome.values.ravel()
    res = view.copy()
    printer.print("Features before filter: " + str(len(res.columns)))
    res = filter_anova(res, outcome_array, p_val=p_val, n_proc=n_proc, printer=printer)
    printer.print("Features after any NaN, zero variance and ANOVA filters: " + str(len(res.columns)))
    return res


def filter_views(views, outcome: DataFrame, printer: Printer, p_val=DEFAULT_P_VAL, n_proc=1):
    res = {}
    for v in views:
...
def filter_view_pre_cv(view: Table, printer: Printer) -> Table:
    progress_observer_factory = SmartProgressObserverFactory(printer=printer, minutes_of_quiet=1)
    res = view.remove_zero_var_cols(progress_observer_factory=progress_observer_factory)
    printer.print("Features after zero variance filter: " + str(n_col(res)))
    return res


def filter_views_pre_cv(views: Views, printer: Printer) -> Views:
    ...
    for v in views.keys():
        printer.print("Filtering view " + v)
        res[v] = filter_view_pre_cv(view=views.view(key=v), printer=printer)
...
def compute_active_features_sv(view, y: DataFrame, printer: Printer, n_proc=1) -> List[bool]:
    ...
    res = filter_anova_mask(view, outcome_array, n_proc=n_proc, printer=printer)
