...
BRCA_MRNA_AGE_NICK = "brca_mrna_age"


class BrcaMrnaAgeInputCreator(InputCreator):

    def __init__(self):
        InputCreator.__init__(self, nick=BRCA_MRNA_AGE_NICK)
    ...
    def inner_create(self, views_to_load: AdjustedViewDef, printer: Printer) -> InputData:
        ...
        outcome_pam50 = pheno.BRCA_Subtype_PAM50.astype('category')
        ...
        outcome_survival = pheno[['vital_status', 'days_to_death', 'days_to_last_followup']]
        outcome_survival = outcome_survival.rename(columns={'vital_status': SURVIVAL_EVENT_STR})
        outcome_survival[SURVIVAL_EVENT_STR] = (outcome_survival[SURVIVAL_EVENT_STR] == "Dead").astype(int)
        ...
        return InputData.smart_create(
            all_views=views, outcomes=outcomes, nick=self.nick(),
            stratify_outcome=OUTCOME_PAM50_NAME, adjusted_views=views_to_load)
