...
from util.dataframe.dataframes import has_column, common_row_names
...
class SmartInputCreator(InputCreator):
    ...
    def inner_create(self, views_to_load: AdjustedViewDef, printer: Printer) -> InputData:
        ...
        common_rows = sorted(common_row_names(dfs=list(views.dataframes()) + [outcomes_df]))
        view_dict_df = views.as_dict_df()
        view_dict_tables = {}
        for v in view_dict_df:
            view_dict_tables[v] = BackedTable(NpTable(data=view_dict_df[v].loc[common_rows]))
        views = JustViews(views_dict=view_dict_tables)
        ...
        return InputData.smart_create(
            all_views=views, outcomes=outcomes, nick=self.nick(), stratify_outcome=stratify_outcome_name,
