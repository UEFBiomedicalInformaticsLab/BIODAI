...
class ClassAndSurvInputCreator(InputCreator):
    ...
    def inner_create(self, views_to_load: AdjustedViewDef, printer: Printer) -> InputData:
        ...
        outcome_categorical = outcome_categorical.rename(columns={self.__class_outcome_col: self.__class_outcome_name})
        ...
        outcomes = [outcome_categorical, outcome_survival]
        views = self._common_preprocessing(views=views, printer=printer)
        return InputData.smart_create(
            all_views=views, outcomes=outcomes, nick=self.nick(), stratify_outcome=self.__class_outcome_name,
