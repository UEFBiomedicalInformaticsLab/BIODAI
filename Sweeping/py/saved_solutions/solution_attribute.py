...
class SolutionAttribute(NickNamed):
    ...
    def read_fold_dfs(self, hof_dir: str) -> Sequence[DataFrame]:
        ...
        It does not read data from final optimization or inner-cv of external validation."""
    ...
    def test_data(self, hof_dir: str, verbose: str = False) -> Optional[Sequence[DataFrame]]:
    ...
    def external_df(self, hof_dir: str) -> Optional[DataFrame]:
        """Returns a df with only the external performances for each objective."""
        df = self.hof_external_solutions_df(hof_dir=hof_dir)
        res = _fold_dfs_external_only(fold_dfs=[df])[0]
        if n_col(res) > 0:
            return res
        else:
            res = _fold_dfs_test_only(fold_dfs=[df])[0]  # For legacy code that saves external as "test".
            if n_col(res) > 0:
                return res
            else:
                return None
    ...
    def train_data(self, hof_dir: str, verbose: bool = False) -> Optional[Sequence[DataFrame]]:
        ...
        If the hof is for internal k-fold CV, returns data from inner cv if available,
