...
from util.dataframe.dataframes import row_as_list
...
def n_folds_from_files(hof_dir: str) -> int:
    ...
    test_fits = FITNESS.test_data(hof_dir=hof_dir)
...
def solutions_from_files(hof_dir: str, verbosity: int = 1) -> Sequence[Sequence[SavedSolution]]:
    ...
    try:
        ...
        print("Cannot read train std deviation. Will try to proceed.")
        if verbosity > 1:
            print("Original exception:\n" + str(e))
        train_sds = None
    try:
        test_sds = STD_DEV.test_data(hof_dir=hof_dir)
    except ValueError as e:
        print("Cannot read test std deviation. Will try to proceed.")
        if verbosity > 1:
            print("Original exception:\n" + str(e))
        test_sds = None
    try:
        ...
        print("Cannot read train CI mins. Will try to proceed.")
        if verbosity > 1:
            print("Original exception:\n" + str(e))
        train_ci_mins = None
    try:
        test_ci_mins = CI_MIN.test_data(hof_dir=hof_dir)
    except ValueError as e:
        print("Cannot read test CI mins. Will try to proceed.")
        if verbosity > 1:
            print("Original exception:\n" + str(e))
        test_ci_mins = None
    try:
        ...
        print("Cannot read train CI maxs. Will try to proceed.")
        if verbosity > 1:
            print("Original exception:\n" + str(e))
        train_ci_maxs = None
    try:
        test_ci_maxs = CI_MAX.test_data(hof_dir=hof_dir)
    except ValueError as e:
        print("Cannot read test CI maxs. Will try to proceed.")
        if verbosity > 1:
            print("Original exception:\n" + str(e))
        test_ci_maxs = None
    ...
    for fold_i, test_fold_fits in enumerate(test_fits):
        if not isinstance(test_fold_fits, DataFrame):
            raise ValueError("test_fold_fits is not a DataFrame")
