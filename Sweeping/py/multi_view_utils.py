...
from util.dataframe.dataframes import prefix_all_cols, nan_slice, select_by_row_indices
from util.distribution.distribution import Distribution, ConcreteDistribution
from util.list_like import ListLike


def view_prefix(view_index: int) -> str:
    return str(view_index) + PREFIX_CONNECTOR


def prefix_a_col(view_index: int, col_name: str) -> str:
    return view_prefix(view_index) + col_name


def prefix_all(view_index: int, col_names: Iterable[str]) -> Sequence[str]:
    prefix = view_prefix(view_index=view_index)
    return [prefix + col_name for col_name in col_names]


def collapse_views(views: dict[str, DataFrame], verbose=False) -> DataFrame:
    """Preserves view order from passed dict."""
...
def collapse_feature_importance_seq(distributions: Sequence[Distribution]) -> Distribution:
    """Each distribution is multiplied by its len / total len, so the probability of selecting each view
    is proportional to its number of features."""
    tot_len = 0
...
def collapse_feature_importance_dict(distributions: dict[str, Distribution]) -> Distribution:
    """Each distribution is multiplied by its len / total len, so the probability of selecting each view
    is proportional to its number of features."""
    tot_len = 0
...
def filter_by_mask(x: DataFrame, mask: ListLike) -> DataFrame:
...
def collapse_views_and_filter_by_mask(views: dict[str, DataFrame], mask: ListLike) -> DataFrame:
