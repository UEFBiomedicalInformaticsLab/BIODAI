...
from hyperparam_manager.sv_hyperparam_manager.dummy_hp_manager import DummyHpManager
...
def assign_crowd_dist_all_fronts(individuals: List[PeculiarIndividual]):
...
def sum_of_individuals(individuals: Sequence[Individual],
    ...
    if len(individuals) == 0:
        ...
        for i in individuals:
            i_features = hp_manager.collapsed_used_features_mask(i)
...
def average_individual(individuals: Sequence[Individual],
                       hp_manager: HyperparamManager = DummyHpManager()) -> numpy.ndarray:
    if len(individuals) == 0:
        return numpy.zeros(0, int)
...
def assign_peculiarity(individuals: List[PeculiarIndividual], hp_manager: HyperparamManager):
    ...
    if average_sum < 0.0:
        ...
        for i in individuals:
            ...
            denominator = max(average_sum, hp_manager.n_predictive_features(i))
            if denominator == 0.0:
...
def assign_just_social_space(individuals: List[PeculiarIndividual]):
