...
from cross_validation.multi_objective.multi_objective_cross_validation import optimize_with_evaluation, \
    do_final_optimization
from cross_validation.multi_objective.optimizer.prefiltered_mo_optimizer_including_fi import ActiveFeaturesCsvWriter
...
from setup.setup_utils import setup_to_folds, save_config_file
...
class SetupRunnerCV(SetupRunner):
    ...
    def run_one_setup(self, setup: EvaluationSetup, printer: Printer, config_file: str = None):
        ...
        save_path = DEFAULT_LOCATION_MANAGER.save_path_from_components(
            ...
            views_to_use=input_data.adjusted_view_def(),
        ...
        # save_path includes the path part from the name of the dataset to the seed, excluding the optimizer.
        ...
        printer.print_variable(var_name="Optimizer save path", var_value=optimizer_save_path)
        ...
        if setup.final_optimization():
            active_feature_observers = [ActiveFeaturesCsvWriter(
                csv_file_path=os.path.join(optimizer_save_path, "active_features_final.csv"))]
            do_final_optimization(input_data=input_data,
                                  mo_optimizer=mo_optimizer.optimizer_for_all_data(
                                      active_feature_observers=active_feature_observers),
                                  objectives=objectives,
