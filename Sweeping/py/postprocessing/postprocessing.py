...
class ViewContributionsPlotsPostprocessing(Postprocessing):
    ...
    def inner_run_postprocessing(self, optimizer_dir: str, main_hofs_dir: str):
        view_contributions_every_hof(
            main_hofs_dir=main_hofs_dir, view_names=views_from_saved_setup(optimizer_dir=optimizer_dir))
...
def run_postprocessing_archive(
        optimizer_dir: str, archive: Sequence[Postprocessing], printer: Printer = OutPrinter()):
    for p in archive:
...
def run_postprocessing_archive_cv_and_final(optimizer_dir: str, printer: Printer = OutPrinter()):
    run_postprocessing_archive(
        optimizer_dir=optimizer_dir, archive=POSTPROCESSING_ARCHIVE_CV, printer=printer)
