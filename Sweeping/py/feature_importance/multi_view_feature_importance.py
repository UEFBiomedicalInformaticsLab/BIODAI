...
class ConcreteMultiViewFeatureImportance(MultiViewFeatureImportance):
    ...
    def compute(
        ...
        if input_data.needs_adjustment():
            raise ValueError("Input data needs to be adjusted first.")
        ...
        y = input_data.outcomes()[0]
        res = {}
        for v in views:
            printer.title_print("Computing feature importance for view " + str(v))
            res_v = self.__fi.compute(x=views[v], y=y, n_proc=n_proc, printer=printer)
            res[v]=res_v
        return res
...
class MVFeatureImportanceUniNB(ConcreteMultiViewFeatureImportance):

    def __init__(self):
        ConcreteMultiViewFeatureImportance.__init__(self, feature_importance=FeatureImportanceUnivariateNB())

    def __str__(self) -> str:
        return "multi-view feature importance univariate NB"
