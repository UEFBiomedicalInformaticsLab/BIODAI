...
from util.math.list_math import list_div, mean_all_vs_others
...
from validation_registry.allowed_property_names import STABILITY_BY_WEIGHTS_NAME, \
    STABILITY_BY_SPEARMAN_NAME, STABILITY_BY_TOP50_NAME, STABILITY_BY_SET_OVERLAP_NAME


def stability_by_weights_from_counts(
        counts: list[Sequence[Union[int, float]]],
...
def stability_by_unions_from_counts(counts: list[Sequence[int]],
    ...
    """Stability by union is equivalent to Dice score when the cardinalities (len of the elements of counts)
    are all equal."""
    if not same_len(counts):
        raise ValueError()
    distributions = []
...
def stability_by_weights(non_dominated_solutions: Sequence[MultiObjectiveOptimizerResult]) -> float:
    counts = []
...
def stability_by_unions_from_individuals(non_dominated_solutions: Sequence[MultiObjectiveOptimizerResult]) -> float:
    ...
    Given sets A and B, with A&B the intersection, it results in |A&B|/max(|A|,|B|). It is equal to the Dice
    coefficient when the cardinalities are equal because if |A|=|B| then |A&B|/max(|A|,|B|)=2|A&B|/(|A|+|B|)."""
...
def stability_by_top_k(non_dominated_solutions: Sequence[MultiObjectiveOptimizerResult], k: int) -> float:
    ...
    for optimizer_result in non_dominated_solutions:  # Loop on folds
        ...
        top_mask = top_k_mask(elems=avg_ind, k=k)
...
def stability_by_spearman(non_dominated_solutions: Sequence[MultiObjectiveOptimizerResult]) -> float:
    fold_feature_weights = []
...
class FeatureStabilityMOCrossEval(MultiObjectiveCrossEvaluator):
    ...
    def evaluate(self, input_data: InputData, folds,
        ...
        hof_registry.set_property(name=STABILITY_BY_WEIGHTS_NAME, value=stab_by_weights)
        hof_registry.set_property(name=STABILITY_BY_SET_OVERLAP_NAME, value=stab_by_unions)
        hof_registry.set_property(name=STABILITY_BY_SPEARMAN_NAME, value=stab_by_spearman)
        hof_registry.set_property(name=STABILITY_BY_TOP50_NAME, value=stab_by_top)
    ...
    def name(self) -> str:
        return "multi-objective feature stability cross-evaluator"
