...
def objective_priority(objective: PersonalObjective) -> int:
    return objective_computer_priority(objective=objective.objective_computer())


def objective_computer_priority(objective: ObjectiveComputer) -> int:
    nick = objective.nick()
    if (nick == Leanness().nick() or
            nick == SoftLeanness().nick() or
            nick == RootLeanness().nick()):
        return 1
    if objective.is_structural_objective_computer():
        return 2
    if objective.is_survival_objective_computer():
        return 3
    if objective.is_class_objective_computer():
        return 4
    return 5  # Just in case.


def objective_computer_pairs_to_plot(
        objectives: Sequence[ObjectiveComputer]) -> Generator[Tuple[int, int, bool], None, None]:
    """TODO At the moment this method works only if the passed objectives are in the same order in which they are
    saved in the HoF. The order instead should be read from disk."""
    n_objectives = len(objectives)
    for i in range(n_objectives):
        for j in range(n_objectives):
            if i != j:
                priority_i = objective_computer_priority(objective=objectives[i])
                priority_j = objective_computer_priority(objective=objectives[j])
                if priority_i <= priority_j:
                    interpolate = (priority_i <= 1)
                    yield i, j, interpolate


def objective_pairs_to_plot(
        objectives: Sequence[PersonalObjective]) -> Generator[Tuple[int, int, bool], None, None]:
    return objective_computer_pairs_to_plot(objectives=[o.objective_computer() for o in objectives])
...
class TwoObjectivesCrossPlot(MultiObjectiveCrossEvaluator):
    ...
    def evaluate(self, input_data: InputData, folds,
        ...
        printer.print("Creating plots for objective pairs using up to " + str(n_proc) + " processes.")

        for i, j, interpolate in objective_pairs_to_plot(objectives=self.__objectives):
            try:
                self.__evaluate_pair(
                    ...
                    interpolate=interpolate, n_proc=n_proc)
