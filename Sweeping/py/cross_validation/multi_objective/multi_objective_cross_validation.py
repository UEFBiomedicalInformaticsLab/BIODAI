...
from consts import DEFAULT_RECURSION_LIMIT, FOLD_RES_DATA_PREFIX, FOLD_RES_DATA_EXTENSION, FEATURE_COUNTS_PREFIX, \
    FEATURE_COUNTS_EXTENSION, FINAL_STR, VALIDATION_REGISTRY_FILE_NAME
...
from cross_validation.multi_objective.cross_evaluator.hof_saver import save_hof_features, \
    SOLUTION_FEATURES_PREFIX, CSV_EXTENSION, save_hof_fitnesses_with_confidence
...
from cross_validation.multi_objective.optimizer.multi_objective_optimizer import MultiObjectiveOptimizer, \
    individual_to_line, mo_result_feature_string_by_indices
...
from ga_components.feature_counts_saver import FeatureCountsSaver, CsvFeatureCountsSaver, DummyFeatureCountsSaver
from ga_components.logbook_saver import LogbookSaver, CsvLogbookSaver, DummyLogbookSaver
...
from location_manager.path_utils import create_optimizer_save_path, path_for_saves
...
from util.randoms import set_all_seeds, SeedIterable
from util.sequence_utils import transpose
from util.str_utils import pretty_duration
from validation_registry.clean_all import clean_all_registries_cv
...
def print_selected_features_all_results(views: Views, hofs: Sequence[MultiObjectiveOptimizerResult], printer: Printer):
    for h in hofs:
        printer.title_print(h.name())
        print_selected_features(views=views, hyperparams=h.hyperparams(), printer=printer)


def print_selected_features_all_hofs(views: Views, hofs: Sequence[HallOfFame], printer: Printer):
    for h in hofs:
        printer.title_print(h.name())
        print_selected_features(views=views, hyperparams=h.hofers(), printer=printer)


def print_selected_features(views: Views, hyperparams: Sequence[IndividualWithContextMV], printer: Printer,
                            skip_big_lists: bool = True):
    feature_names = views.collapsed().colnames()
    ...
    printer.print(mo_result_feature_string_by_indices(indices=sorted_feature_pos, feature_names=feature_names))
...
class MultiObjectiveOptimizeOnFolds(ABC):
    ...
    @staticmethod
    def create_fold_printer(save_path: Union[str, None], fold_index: int) -> Printer:
        str_f = str(fold_index)
        tagged_printer = TaggedPrinter(tag="Fold " + str_f, inner=UnbufferedOutPrinter())
        if save_path is None:
            return tagged_printer
        else:
    ...
    @staticmethod
    def create_fold_logbook_saver(save_path: Union[str, None], fold_index: int) -> LogbookSaver:
        if save_path is None:
            return DummyLogbookSaver()
        else:
            str_f = str(fold_index)
            return CsvLogbookSaver(file=save_path + FOLD_RES_DATA_PREFIX + str_f + "." + FOLD_RES_DATA_EXTENSION)

    @staticmethod
    def create_fold_feat_counts_saver(save_path: Union[str, None], fold_index: int, save_interval: int = 1
                                      ) -> FeatureCountsSaver:
        if save_path is None:
            return DummyFeatureCountsSaver()
        else:
    ...
    @staticmethod
    def create_fold_worker_printer(save_path: Union[str, None], fold_index: int) -> Printer:
        str_f = str(fold_index)
        tagged_printer = TaggedPrinter(tag="Fold " + str_f, inner=UnbufferedOutPrinter())
        if save_path is None:
            return tagged_printer
    ...
    @staticmethod
    def fold_specific_execution(fold_input: FoldSpecificInput) -> Sequence[MultiObjectiveOptimizerResult]:
    ...
    def create_fold_inputs(input_data: InputData, folds: Folds, mo_optimizer_by_fold: MultiObjectiveOptimizerByFold,
                           save_path: Union[str, None], proc_per_fold=1
                           ) -> Iterator[FoldSpecificInput]:
        """Creates a seed iterable using current random, uses it to assign a different seed to each fold input."""
        n_folds = folds.n_folds()
        fold_input_seed = iter(SeedIterable())
        return (FoldSpecificInput(
                fold_index=f,
                save_path=save_path,
                mo_optimizer=mo_optimizer_by_fold.optimizer_for_fold(fold_index=f),
                train_data=input_data.select_samples(row_indices=folds.train_indices(fold_number=f)).serialize(),
                fold_processes=proc_per_fold,
                seed=next(fold_input_seed)) for f in range(n_folds))
...
class MultiObjectiveOptimizeOnFoldsSerial(MultiObjectiveOptimizeOnFolds):
    ...
    def optimize(self, input_data: InputData, folds: Folds, mo_optimizer_by_fold: MultiObjectiveOptimizerByFold,
                 save_path: Union[str, None], n_proc=1
                 ) -> list[list[MultiObjectiveOptimizerResult]]:
...
class MOEvaluationRes:
    ...
    __final_optimization: Sequence[MultiObjectiveOptimizerResult]

    def __init__(self, evaluation_res: list, final_optimization: Sequence[MultiObjectiveOptimizerResult]):
    ...
    def final_optimization(self) -> Sequence[MultiObjectiveOptimizerResult]:
...
def create_final_optimizer_printer(optimizer_save_path: str, additional_printer: Printer = NullPrinter()) -> Printer:
    ...
    return CompositePrinter([UnbufferedOutPrinter(), log_printer])
...
def do_final_optimization(
        ...
        n_proc=1) -> Sequence[MultiObjectiveOptimizerResult]:
    ...
    printer.print_variable("optimizer save path",optimizer_save_path)
    ...
    printer.print("Removing adjustment information from input data: making all views predictive.")
    input_data = input_data.make_all_views_predictive()
    ...
    for hof in final_optimization:
        ...
        printer.print("Saving HoF " + hof.name() + " to directory " + path_saves)
        save_hof_features(path_saves=path_saves,
                          file_name=SOLUTION_FEATURES_PREFIX + FINAL_STR + CSV_EXTENSION,
                          feature_names=input_data.collapsed_feature_names(),
                          hof=hof)
        save_hof_fitnesses_with_confidence(
            path_saves=path_saves,
            fitnesses_file_name="solution_fitnesses_" + FINAL_STR + ".csv",
            std_dev_file_name="solution_std_devs_" + FINAL_STR + ".csv",
            ci_min_file_name="solution_ci_min_" + FINAL_STR + ".csv",
            ci_max_file_name="solution_ci_max_" + FINAL_STR + ".csv",
            hof=hof,
...
def optimize_with_evaluation(input_data: InputData,
    ...
    printer = CompositePrinter([UnbufferedOutPrinter(), log_printer])
    ...
    clean_all_registries_cv(optimizer_save_path=optimizer_save_path, folds_hofs=folds_hofs)
    ...
    for evaluator in mo_evaluators:
        for hof in folds_hofs:
            hof_name = hof[0].name()
            printer.title_print("Applying " + evaluator.name() + " to hall of fame " + hof_name)
            ...
            try:
                ...
                printer.print("Exception caught while running an evaluator. Program will try to continue.")
                printer.print_variable(var_name="Hall of fame", var_value=hof_name)
                printer.print_variable(var_name="Evaluator", var_value=evaluator.name())
                printer.print("Exception:")
                printer.print(str(e))
                printer.print_variable("traceback", traceback.format_exc())
