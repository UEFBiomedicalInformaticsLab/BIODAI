...
from model.regression.regressors_library import SVRegressorModel
from objective.objective_with_importance.adjusted_objective_computer import AdjustedObjectiveComputer
...
from util.randoms import random_seed, random_state_context_blended
from util.sequence_utils import transpose
from util.str_utils import pretty_duration, name_value, iterable_to_string
...
DEFAULT_ADJUSTER_REGRESSOR = SVRegressorModel()
...
class AdjustedOptimizer(MultiObjectiveOptimizerAcceptingFeatureImportance):
    ...
    def _train_adjuster_one_objective(
        ...
        for fold_index in range(n_folds):
            ...
            all_fitnesses = evaluate_objective_for_fold_with_inner_cv(
                    ...
                    train_data=input_data.select_samples(row_indices=fold[0]),
                    test_data=input_data.select_samples(row_indices=fold[1]),
                    compute_ci=False,
                    n_proc=n_proc)
    ...
    def optimize_with_feature_importance(self, input_data: InputData, printer: Printer,
                                         feature_importance: dict[str,Distribution], n_proc=1,
        ...
        with random_state_context_blended(additional_seed=87432, printer=printer):
            ...
            if tuning_hof is None:
                raise ValueError(
                    ...
                    iterable_to_string(li=[hof[0].nick() for hof in folds_hofs], brackets=False))
            ...
            if self.__precog:
                ...
                for all_fold_results, fold in zip(all_results, folds_list):
                    fit_by_objective = evaluate_all_objectives_for_fold_with_inner_cv(
                        ...
                        train_data=input_data.select_samples(row_indices=fold[0]),
                        test_data=input_data.select_samples(row_indices=fold[1]),
                        compute_ci=False, n_proc=n_proc)
                    test_fold_results = []
            ...
            for obj_i in range(len(self.__objectives)):
                ...
                adjusted_objective_computer = AdjustedObjectiveComputer(
