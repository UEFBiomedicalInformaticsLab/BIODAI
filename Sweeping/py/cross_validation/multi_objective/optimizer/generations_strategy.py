...
class GenerationsStrategy(NickNamed):
    ...
    def nick(self) -> str:
        ...
        if self.num_sweeps() > 0:
            res += iterable_to_string(self.__sweeps, compact=True)
    ...
    def __str__(self) -> str:
        res = ""
        if self.num_sweeps() > 0:
            res += iterable_to_string(self.__sweeps)
        if self.concatenated_generations() > 0:
            if res != "":
                res += " "
            res += str(self.concatenated_generations())
    ...
    def __eq__(self, other) -> bool:
        if self is other:
            return True
        if not isinstance(other, GenerationsStrategy):
            return False
        return self.__concatenated == other.__concatenated and self.__sweeps == other.__sweeps
