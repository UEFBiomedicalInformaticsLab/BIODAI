...
class GuidedForward(MultiObjectiveOptimizer):
    ...
    def create_individual(features: Sequence[str],
        ...
        res_list = SparseBoolListBySet(min_size=individual_updater.individual_size())
    ...
    def create_from_weights(
        ...
        sorting = sort_permutation(weights, in_reverse=True)
        non_zero_sorting = []
        for i in sorting:
            if weights[i] > 0.0:
    ...
    def create_from_fi(
        ...
        weights = collapse_feature_importance_dict(distributions=feature_importance)
