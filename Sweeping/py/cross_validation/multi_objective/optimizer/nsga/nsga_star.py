...
from util.str_utils import name_value, pretty_duration
...
class NsgaStar(MultiObjectiveOptimizerAcceptingFeatureImportance):
    ...
    def optimize_with_feature_importance(
        ...
        printer.title_print("Initializing genetic algorithm")
        printer.print("Creating GA runner")
        ...
        printer.print("Creating population observers")
        hofs = [h.create_population_observer() for h in self.__hof_factories]

        if len(known_solutions) > 0:
            printer.print("Starting from known solutions")
            initial_pop = [k.modifiable_copy() for k in known_solutions]
        else:
            printer.print("Starting from scratch without known solutions")
            initial_pop = None
        ...
        n_eval = self.__n_gen * self.__pop_size
        printer.print(
            "Running the genetic algorithm took " + str(pretty_duration(ga_time)) +
            " (" + "{:.2f}".format(ga_time / n_eval) + " seconds per evaluation)" )
        printer.title_print("Finished running genetic algorithm, collecting results")
        printer.print(ga_result.logbook)
        logbook_saver.save(ga_result.logbook)
        feature_names = collapsed_views.colnames()
