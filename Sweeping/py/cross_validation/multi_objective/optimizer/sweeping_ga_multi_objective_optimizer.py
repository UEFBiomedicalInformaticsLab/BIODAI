...
from cross_validation.multi_objective.optimizer.multi_objective_optimizer import hofers_to_results
...
from run_ga.run_ga_2_steps import run_sweeps
...
class SweepingGAMultiObjectiveOptimizer(MultiObjectiveOptimizerAcceptingFeatureImportance):
    """Can use either a resampling master or a concatenated master."""
    __objectives: Iterable[PersonalObjective]
    __sorting_strategy: SortingStrategy
    __nick: str
    __name: str
    __sweeping_strategy: GenerationsStrategy
    ...
    def __init__(self, pop_size, mutation_frequency, mating_prob,
        ...
        self.__master_runner = master_runner
        self.__optimizer_type = ConcreteMOOptimizerType(
            uses_inner_models=True, nick=self.__master_runner.sweeping_type_nick(),
            name="Sweeping genetic algorithm multi-view multi-objective optimizer with " + self.__master_runner.name())
    ...
    def optimize_with_feature_importance(self, input_data: InputData, printer: Printer,
        ...
        """known_solutions are ignored at the moment."""
        sweeping_strategy = self.__sweeping_strategy
        ga_result = run_sweeps(
            input_data=input_data, pop_size=self.__pop_size,
            mutating_prob=self.__mutation_frequency,
            mating_prob=self.__mating_prob,
            ...
            bitlist_mutation=self.__mutation,
            ...
            master_runner=self.__master_runner,
            seed=random_seed()
        )
