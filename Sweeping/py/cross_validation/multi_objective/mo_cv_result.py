...
from util.preconditions import check_has_none
...
class MOCVResult:
    ...
    def __init__(self, so_results: Sequence[CVResult]):
        self.__so_results = [deepcopy(so) for so in check_has_none(so_results)]
        importances = []
    ...
    def __str__(self) -> str:
        ...
        for so in self.__so_results:
            res += str(so)
        if self.has_importances():
            res += "Feature importances: " + iterable_to_string(self.importances()) + "\n"
        return res
