...
from ga_runner.ga_progress_observer import LazyGAProgressObserver
...
from util.str_utils import str_sorted_dict, str_in_lines


def write_pop(individuals: Sequence[PeculiarIndividual]) -> str:
...
def average_fitnesses(individuals: Sequence[PeculiarIndividual]) -> Optional[Sequence[float]]:
...
def num_features_counts(individuals: Sequence[PeculiarIndividual]) -> dict[int, int]:
    res = {}
    n_features = [sum(i) for i in individuals]
    for n in n_features:
        if n in res:
...
class PopStatsObserver(LazyGAProgressObserver):
    __printer: Printer
...
class PopWriterObserver(LazyGAProgressObserver):
...
class PopNumFeaturesObserver(LazyGAProgressObserver):
    __printer: Printer
