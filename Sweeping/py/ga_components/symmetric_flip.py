...
def symmetric_flip(individual: Individual, frequency: float):
    """ The frequency of reducing mutations is equal to the frequency of increasing mutations.
        Frequency 1 means 1 mutation on average. Frequency can be higher than one.
    ...
    if ind_ones == 0:
        prob_1_to_0 = 0.0  # Not relevant since there are no ones to convert.
    else:
        prob_1_to_0 = min(frequency/(2.0*ind_ones), 1.0)
    if ind_zeros == 0:
        prob_0_to_1 = 0.0  # Not relevant since there are no zeros to convert.
    else:
...
def symmetric_flip_with_mask(individual: Individual, frequency: float, active_mask: Optional[ListLike] = None):
    """ The frequency of reducing mutations is equal to the frequency of increasing mutations.
        This function uses the :func:`~random.random` function from the python base
        :mod:`random` module.
    ...
    if active_mask is None:
        ...
        if ind_zeros == 0:
            ...
            prob_0_to_1 = min(frequency / (2.0 * ind_zeros), 1.0)
        for i in active_mask.true_positions():
            ind_i = individual[i]
            if ind_i:
                if random() < prob_1_to_0:
