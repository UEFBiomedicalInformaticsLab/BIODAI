...
from individual.peculiar_individual import PeculiarIndividualSV
...
class Nsga3(PopSorter):
    ...
    def __init__(self, num_objectives: int, max_reference_points: int):
        """NSGA3 requires at least two objectives.
        It is possible to solve the issue in the init function bt setting max_reference_points to 1,
        but then there is an exception during execution in associate_to_niche."""
        if num_objectives < 2:
            raise ValueError("NSGA3 requires at least two objectives.")
    ...
    def sort_front(self, all_fitnesses: ndarray, front_fitnesses: ndarray, front: Sequence[PeculiarIndividualSV]
                   ) -> Sequence[PeculiarIndividualSV]:
    ...
    def sort(self, pop: Sequence[PeculiarIndividualSV], hp_manager: SvHyperparamManager) -> Sequence[PeculiarIndividualSV]:
