...
class SorterWithCloneIndex(PopSorter):
    ...
    @staticmethod
    def to_tuple(ind: PeculiarIndividual, hp_manager: HyperparamManager) -> tuple:
        return hp_manager.to_tuple(ind)

    def ci_fronts(
            self,
            pop: Sequence[PeculiarIndividual], hp_manager: HyperparamManager) -> Sequence[Sequence[PeculiarIndividual]]:
        ci_fronts = []
        tuple_to_ci = {}
        for i in pop:
