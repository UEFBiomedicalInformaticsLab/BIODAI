...
def barplot_with_std_ax(
    ...
    bar_names = labels_transformer.apply_all(bar_names)
    ...
    if horizontal:
        # We have to reverse the bars otherwise they are drawn bottom-up.
        ax.barh(ticks_pos, reverse(bar_lengths), xerr=reverse(bar_std), zorder=2, color=reverse(colors))
        ax.set_yticks(ticks_pos)
        ax.set_yticklabels(reverse(bar_names), rotation=rotation, fontsize=font_size)
