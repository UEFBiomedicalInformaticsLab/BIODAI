...
from plots.plot_utils import single_line_plot_to_path, multi_line_plot_ax, smart_save_fig, DEFAULT_DECIMALS
...
def remove_dominated(x: Sequence[float], y: Sequence[float]) -> tuple[Sequence[float], Sequence[float]]:
    res_x = []
    res_y = []
...
def single_line_from_sequences(x: Sequence[Sequence[float]], y: Sequence[Sequence[float]],
                               name_x: str, name_y: str) -> tuple[Sequence[float], Sequence[float]]:
...
def single_front_plot_from_sequences(x: Sequence[Sequence[float]], y: Sequence[Sequence[float]],
                                     label_x: str, label_y: str, hof_dir: str):
...
def multi_front_plot_ax(
        ax: Axes, dfs: Sequence[Sequence[DataFrame]], col_x: int, col_y: int, col_name_x: str, col_name_y: str,
    ...
    multi_line_plot_ax(
        ...
        line_labels=names, x_min=x_min, x_max=x_max, y_min=y_min, y_max=y_max, font_size=font_size, decimals=decimals,
        x_tick_labels=x_tick_labels, legend_loc=legend_loc)


def multi_front_plot(dfs: Sequence[Sequence[DataFrame]], i: int, j: int,
                     col_name_x: str, col_name_y: str, save_path: str,
                     names=Optional[Sequence[str]], x_label: str = None, y_label: str = None,
                     x_min: float = None, y_min: float = None,
...
def single_front_plot(dfs: Sequence[DataFrame], i: int, j: int, label_i: str, label_j: str, hof_dir: str):
...
def sequence_vals_to_labels(s: Sequence[float], label: str) -> Sequence[float]:
    """TODO Refactor to automatically use new objectives."""
    root_leanness = RootLeanness()
    if label.endswith(root_leanness.nick()):
        return root_leanness.vals_to_labels(s)
    soft_leanness = SoftLeanness()
    if label.endswith(soft_leanness.nick()):
        return soft_leanness.vals_to_labels(s)
    leanness = Leanness()
...
def single_front_plots(hof_dir: str, verbose: bool = False):
    dfs = FITNESS.test_data(hof_dir=hof_dir)
    if dfs is None:
        if verbose:
            print("Impossible to create test dataframe from directory " + str(hof_dir))
    else:
...
def multi_front_plot_from_dirs(hofs_dir: Sequence[str], save_path: str, names=Optional[Sequence[str]],
    ...
    for i in range(len(hofs_dir)):
        ...
        if names is not None:
            name = names[i]
        if os.path.isdir(f):
            tdfs = FITNESS.test_data(hof_dir=f)
            if tdfs is not None:
...
def multi_front_plot_from_saved_hofs(saved_hofs: Sequence[SavedHoF], save_path: str,
    ...
    """Legacy function to be removed when safe."""
    ...
    multi_front_plot_from_dirs(hofs_dir=hofs_dir, save_path=save_path, names=names, x_label=x_label, y_label=y_label,
                               x_min=x_min, x_max=x_max, y_min=y_min, y_max=y_max, legend_loc=legend_loc)
