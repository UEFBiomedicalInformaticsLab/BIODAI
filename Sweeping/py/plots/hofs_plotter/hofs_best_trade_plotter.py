    ...
    def plot(self, ax, saved_hofs: Sequence[SavedHoF], color=None):
        ...
        for s in saved_hofs:
            ...
            if hof_dfs is not None:
                dfs.append(hof_dfs)
                name_parts.append(s.name_parts())
        setup = self._setup()
        ...
        if len(dfs) > 0:
            ...
            names = setup.labels_map().apply_all(names_by_differences(object_features=name_parts))
            multi_front_plot_ax(
                ax=ax, dfs=dfs,
                ...
                x_label=x_label, y_label=y_label, decimals=setup.decimals(), legend_loc=setup.legend_loc())
