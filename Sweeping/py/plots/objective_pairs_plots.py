...
from plots.hofs_plotter.plot_setup import DEFAULT_SPLINES
from plots.monotonic_front import df_vals_to_labels
from plots.plot_utils import smart_save_fig, DEFAULT_DECIMALS
from plots.saved_hof import SavedHoF
from util.table.table_utils import n_col
from util.plot_results import ADD_ELLIPSES_DEFAULT, multiclass_scatter_to_ax
...
def external_objective_pairs_plot(ax, dfs: Sequence[DataFrame], i: int, j: int, label_i: str, label_j: str,
    ...
    multiclass_scatter_to_ax(
        ...
        font_size=font_size, interpolate=interpolate, add_ellipses=add_ellipses, decimals=decimals)
...
def one_objective_pair_plot_from_saved_hofs(
        ...
        verbose: bool = False, decimals: Optional[int] = DEFAULT_DECIMALS, splines: bool = DEFAULT_SPLINES):
    algo_dfs = []
    name_parts = []
    for hof in saved_hofs:
        df = hof.to_df(verbose=verbose)
        if df is not None:
            algo_dfs.append(df)
    ...
    if len(algo_dfs) > 0:
        ...
        label_i = labels_map.apply(label=label_i)
        label_j = labels_map.apply(label=label_j)
        names = labels_map.apply_all(names_by_differences(object_features=name_parts))
        external_objective_pairs_plot(
            ax=ax,
            dfs=algo_dfs, i=i, j=j, label_i=label_i, label_j=label_j,
            names=names,
            x_min=x_min, x_max=x_max, y_min=y_min, y_max=y_max, alpha=alpha, font_size=font_size, decimals=decimals,
            interpolate=splines)
