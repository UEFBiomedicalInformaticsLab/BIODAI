...
class GeneBoxplotter(Plotter):
    ...
    def __init__(self, input_data: InputData, feature_names: Sequence[str], standardize: bool = True):
        mrna_view = input_data.view(MRNA_NAME).to_dataframe()
        self.__df = mrna_view[mrna_view.columns.intersection(feature_names).sort_values()].copy()
        if standardize:
            self.__df.loc[:, self.__df.columns] = StandardScaler().fit_transform(self.__df[self.__df.columns])
        if input_data.n_outcomes() > 0:
            outcome = DataFrame({"subtype": ravel(input_data.outcomes()[0].data().values)})
            # Assuming first outcome is the one we need.
        else:
            outcome = DataFrame({"subtype": ["Normal tissue"] * input_data.n_samples()})
            # Assuming if there is no outcome is normal tissue
        # f_column = outcome["subtype"]
        self.__df.reset_index(drop=True, inplace=True)
        outcome.reset_index(drop=True, inplace=True)
        self.__df = pd.concat([self.__df, outcome], axis=1)
    ...
    def plot(self, ax: Axes, color=None):
        ...
        with plt.style.context({'font.size': FONT_SIZE}):
            seaborn.boxplot(x="subtype", y="value", data=melted_df, hue="genes", ax=ax, palette=DEFAULT_PALETTE_NAME)
...
def create_gene_box_plotter_with_normal(input_data_normal: InputData, input_data_tumor: InputData,
    ...
    mrna_view_tumor = input_data_tumor.view(MRNA_NAME).to_dataframe()
    mrna_view_normal = input_data_normal.view(MRNA_NAME).to_dataframe()
    ...
    df = pd.concat([df_normal, df_tumor], ignore_index=True)
