...
from plots.default_labels_map import DEFAULT_LABELS_TRANSFORMER, LabelsTransformer
...
from validation_registry.registry_property_archive import (
    TEST_HV_PROPERTY, CROSS_HV_PROPERTY, MEAN_JACCARD_PROPERTY,
    STABILITY_BY_WEIGHTS_PROPERTY, STABILITY_BY_SET_OVERLAP_PROPERTY, STABILITY_BY_BEST_DICE_PROPERTY,
    INNER_CV_HV_PROPERTY, PERFORMANCE_GAP_PROPERTY, PERFORMANCE_ERROR_PROPERTY, PARETO_DELTA_PROPERTY)

DEFAULT_REGISTRY_PROPERTIES = (
    STABILITY_BY_SET_OVERLAP_PROPERTY, STABILITY_BY_BEST_DICE_PROPERTY, MEAN_JACCARD_PROPERTY,
    STABILITY_BY_WEIGHTS_PROPERTY,
    CROSS_HV_PROPERTY, TEST_HV_PROPERTY, PARETO_DELTA_PROPERTY, INNER_CV_HV_PROPERTY, PERFORMANCE_ERROR_PROPERTY,
    PERFORMANCE_GAP_PROPERTY)


def pre_measure_path_cv(main_plot_dir: str, dataset_report_path_part: str) -> str:
    return main_plot_dir + "/cv/" + dataset_report_path_part + "/"

def pre_measure_path_external(main_plot_dir: str, dataset_report_path_part: str) -> str:
    return main_plot_dir + "/external/" + dataset_report_path_part + "/"
...
def process_property(
    ...
    for h in hofs:
        ...
        if prop.has_folds_property_name():
            if not prop.has_complete_folds_property(registry=registry):
                folds_prop_name = prop.folds_property_name()
                print("Property " + folds_prop_name + " not found or not complete for hall of fame " + h.name() + ".")
                print("The property will be computed and saved.")
...
def summary_statistics_plotter_from_registries(
    ...
    if test_battery.is_external():
        ...
        for d in test_battery.datasets():
            print("Processing external validation " + str(d))
