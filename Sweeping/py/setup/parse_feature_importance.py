...
from feature_importance.multi_view_feature_importance import MVFeatureImportanceUniform, MultiViewFeatureImportance, \
    ...
    MVFeatureImportanceUniCox, MVFeatureImportanceCox, MVFeatureImportanceUniNB
from setup.allowed_names import UNIVARIATE_COX_NAME, UNIVARIATE_NB_NAME
from setup.allowed_names import UNIFORM_NAME, LASSO_NAME, SOFT_LASSO_NAME, ANOVA_NAME, NONE_NAME, COX_NAME
from setup.setup_utils import choose_hof_nick
...
def parse_categorical_fi(feature_importance_str: str = NONE_NAME) -> MultiViewFeatureImportance:
    ...
    elif feature_importance_str == UNIVARIATE_NB_NAME:
        feature_importance = MVFeatureImportanceUniNB()
    else:
...
def parse_feature_importance_by_fold(
    ...
    if isinstance(base_dir, str):
        printer.print("Loading previous hall of fame for feature importance")
        printer.print_variable("base directory", base_dir)
        hof_nick = choose_hof_nick(previous_optimizer_dir=base_dir)
        printer.print_variable("input HoF", hof_nick)
        return FeatureImportanceByPrevious(previous_optimizer_dir=base_dir, previous_hof_nick=hof_nick)
    else:
        categorical_fi = parse_categorical_fi(categorical_fi_str)
        survival_fi = parse_survival_fi(survival_fi_str)
