...
from input_data.input_creator.input_simulator import SIMULATED_NICK, InputSimulator, DEFAULT_N_CLASSES, DEFAULT_N_ROWS, DEFAULT_N_COLS
...
from input_data.input_creator.brca_input_creator import BrcaInputCreator
from input_data.input_creator.input_creators_archive import INPUT_CREATORS_DICT
from input_data.input_data import InputData
from input_data.input_creator.kir_input_creator import KirInputCreator
from input_data.input_creator.luad_lusc_input_creator import LuadLuscInputCreator
from input_data.input_creator.prad_input_creator import PradInputCreator
from input_data.input_creator.swedish_input_creator import SwedishInputCreator
from setup.allowed_names import STRATIFIED_K_FOLD_NAME, LOAD_FOLD_NAME, AUTO_FOLD_NAME, DEFAULT_VIEWS_MV
...
SAVED_CONFIG_FILE_NAME = "config.ini"
...
def create_outer_folds_list(outer_n_folds: int, input_data: InputData, n_repeats: int = 1,
                            ...
                            seed: int = 365) -> list[tuple[IndexArray,IndexArray]]:
...
def load_input_data(dataset_name: str, views_to_use: Optional[AdjustedViewDef] = None,
                    printer: Printer = UNBUFFERED_OUT_PRINTER, skip_plotting_huge_views: bool = True,
    ...
    elif dataset_name == SIMULATED_NICK:
        input_creator = InputSimulator(n_row=DEFAULT_N_ROWS, n_col=DEFAULT_N_COLS, n_classes=DEFAULT_N_CLASSES)
    else:
    ...
    res = input_creator.create(views_to_load=views_to_use, printer=printer,
                               ...
                               covariate_views=covariate_views)
    printer.print("Loaded " + str(res.n_samples()) + " samples.")
    return res
...
def save_config_file(config_file: str, destination_path: str, printer: Printer):
    if config_file is not None:
        ...
        shutil.copy(config_file, destination_path + SAVED_CONFIG_FILE_NAME)
...
def choose_hof_nick(previous_optimizer_dir: str, hofs_priority: Sequence[str] = (PARETO_NICK, LASSO_STR)) -> str:
